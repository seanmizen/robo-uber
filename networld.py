...
class Node:
    ...
    def __init__(self, parent, index, can_stop=True, capacity=1,
        ...
        # index is the node's (x,y) tuple in the network
        self._idx = index
        # reachable neighbours (not necessarily symmetric)
        self._neighbours = [N, NE, E, SE, S, SW, W, NW]
        self._canStop = can_stop                 # taxis can stop here
        # max number of taxis that can be in this point
        self._capacity = capacity
        # dictionary of taxis at this point, indexed by current direction
        self._occupied = {}
        # dictionary of taxis attempting to enter this point
        self._incoming = {}
        # priority management for access. Indexes the direction with first priority
        self._traffic_light = 0
        # _traffic point where the Node becomes locked
        self._trafficMax = traffic_cap
        # amount of traffic automatically generated coming in per clock
        self._trafficSrc = traffic_in
        # amount of traffic automatically removed per clock
        self._trafficSink = -traffic_out
        self._traffic = 0
        self._fare = None
    ...
    def clockTick(self, parent):
        if self._parent == parent:
            ...
            offDuty = [taxi for taxi in self._occupied.items()
                       if not taxi[1][0].onDuty]
            for taxi in offDuty:
    ...
    def turn(self, directionIn, directionOut=-1):
        ...
        self._neighbours[relativeDirection].indicate(
            newDirection, self._occupied[directionIn][0])
    ...
    def vacate(self, directionIn, directionOut=None):
        ...
        newSpace = self._neighbours[relativeDirection].occupy(
            directionOut, self._occupied[directionIn][0], self)
   ...
   '''
        ...
        if interpolate:
            for edge in edges:
                if edge.nodeA not in self._net:
                    raise ValueError(
                        "Node {0} does not exist in the map".format(edge.nodeA))
                if edge.nodeB not in self._net:
                    raise ValueError(
                        "Node {0} does not exist in the map".format(edge.nodeB))
                ...
                if edge.dirA < 2 or edge.dirA > 6:
                    if src.index[1] <= dst.index[1]:
                        raise ValueError("Road exit point from node {0} points away from destination node {1}".format(
                            src.index, dst.index))
                    srcExity -= 1
                ...
                if nextNodeIdx[0] == dst.index[0] and nextNodeIdx[1] == dst.index[1]:
                    if self.addEdgeSegment(src, nextNodeIdx, edge.bidirectional) != dst:
                        raise KeyError("Indexed node ({0},{1}) should be the end junction {2}".format(
                            nextNodeIdx[0], nextNodeIdx[1], dst.index))
                    return
                ...
                if not edge.bidirectional:
                    ...
                    if self.addEdgeSegment(penultimateNode, (dst.index[0], dst.index[1]), False) != dst:
                        raise KeyError("Penultimate node {1} should be adjacent to end junction {2}".format(
                            penultimateNode.index, dst.index))
                # bidirectional cases can be just handled by the ordinary machinery.
                else:
                    penultimateNode = self.addEdgeSegment(
                        dst, penultimateNodeIdx, edge.bidirectional)
                # insert the adjacent node from the origin
                nextNode = self.addEdgeSegment(
                    src, (srcExitx, srcExity), bidir=True)
                ...
                while nextNode != penultimateNode:
                    nextNode = self.addEdgeSegment(
                        nextNode, penultimateNodeIdx, bidir=True)

        # when we don't have to interpolate, the process is a LOT simpler
        else:
            for edge in edges:
                self._net[edge.nodeA].addNeighbour(
                    self, self._net[edge.nodeB], edge.dirA)
                if edge.bidirectional:
                    self._net[edge.nodeB].addNeighbour(
                        self, self._net[edge.nodeA], edge.dirB)
    ...
    def addEdgeSegment(self, start, end, bidir=True):
        ...
        if nextIdx[0] > self.xSize-1 or nextIdx[0] < 0 or nextIdx[1] > self.ySize-1 or nextIdx[1] < 0:
            raise IndexError("Next node out of range: ({0},{1})".format(
                nextIdx[0], nextIdx[1]))
    ...
    def addDispatcher(self, dispatcher):
        ...
        for fare in self._fareQ.values():
            ...
            if fare.taxi is not None:
                self._dispatcher.handover(
                    self, fare.origin, fare.destination, fare.callTime, fare.taxi, fare.price)
            else:
                self._dispatcher.newFare(
                    self, fare.origin, fare.destination, fare.callTime)
    ...
    def insertFare(self, node):
        if node.index in self._fareQ:
            raise IndexError(
                "Node {0} generated a new fare for one where a Fare is already waiting".format(node.index))
        ...
        if self._dispatcher is not None:
            self._dispatcher.newFare(
                self, newFare.origin, newFare.destination, newFare.calltime)
    ...
    def broadcastFare(self, origin, destination, price):
        ...
        for taxi in self._taxis.keys():
            if taxi.onDuty:
                onDuty += 1
                taxi.recvMsg(
                    taxi.FARE_ADVICE, **{'origin': origin, 'destination': destination, 'price': price})
    ...
    def runWorld(self, ticks=0, outputs=None):
        ...
        while (ticks == 0 or ticksRun < ticks) and (self.runTime == 0 or self._time < self.runTime):
            ...
            for node in self._trafficQ.items():
                self._trafficQ[node[0]] -= self._net[node[0]
                                                     ].injectTraffic(self, node[1])
